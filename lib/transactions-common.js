...
Transact.prototype.insert = function (collection, newDoc, opt, callback) {
  ...
  this._insertOne(collection, newDoc, opt, callback);
}

Transact.prototype._insertOne = function (collection, newDoc, opt, callback) {
...
Transact.prototype.remove = function (collection, selector, opt, callback) {
  ...
  // But also need to identify any callback functions
  var callback = (_.isFunction(callback)) ? callback : ((typeof opt !== 'undefined') ? ((_.isFunction(opt)) ? opt : ((_.isFunction(opt.callback)) ? opt.callback : undefined)) : undefined);
  // TODO (not for objsheets): Call the callback once at the correct time for
  // multi-document operations.
  if (!(Meteor.isServer && callback == null))
    throw new Error("Only implemented on the server with no callback");
  if (opt && _.isObject(opt.tx)) {
    opt = opt.tx;
  }
  collection.find(selector).forEach(this._removeOne.bind(this, collection, opt, callback));
  // TODO (not for objsheets): Meaningful return value.
}

Transact.prototype._removeOne = function (collection, opt, callback, existingDoc) {
  if (this._rollback) return;
  var _id = existingDoc._id;
...
// isModificationMod copied from:
// https://github.com/meteor/meteor/blob/cf4133c7632137d870136f02100622aa7e8fcd95/packages/mongo/mongo_driver.js#L591
...
Transact.prototype.update = function (collection, selector, updates, opt, callback) {
  ...
  if (!(Meteor.isServer && callback == null))
    throw new Error("Only implemented on the server with no callback");
  // FIXME (not for objsheets): When opt.tx is an object, are options unrelated
  // to transactions (e.g., multi, or maybe collections2 stuff?) supposed to be
  // passed inside or outside the object?  Make a decision and clarify the
  // documentation.  Potentially similar issue with insert and remove.
  // ~ Matt 2016-12-13
  if (opt && _.isObject(opt.tx)) {
    opt = opt.tx;
  }
  var opt = (_.isObject(opt)) ? _.omit(opt,'tx') : undefined;
  if (isModificationMod(updates)) {
    var findOptions = {};
    if (!(opt && opt.multi))
      findOptions.limit = 1;
    collection.find(selector, findOptions).forEach(this._updateOne.bind(this, collection, updates, opt, callback));
  } else {
    // We do not yet have a way to represent whole-document replacement in the
    // history.  For now, emulate it using remove + insert.
    if (opt && opt.multi)
      throw new Error("Update with a replacement document cannot use multi.");
    var existingDoc = collection.findOne(selector);
    if (existingDoc) {
      this._removeOne(collection, opt, callback, existingDoc);
      var newDoc = _.clone(updates);
...
Transact.prototype._updateOne = function (collection, updates, opt, callback, existingDoc) {
  if (this._rollback) return;
  var _id = existingDoc._id;
